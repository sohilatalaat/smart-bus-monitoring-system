...
# Buttons: ARRIVE (pin 4)  |  DEPART (pin 5)
# ============================================================
...
ARRIVE_BUTTON_PIN = 4           # arrive now (skip travel time)
DEPART_BUTTON_PIN = 5           # depart now (leave the stop early)
...
arrive_button = Pin(ARRIVE_BUTTON_PIN, Pin.IN, Pin.PULL_UP)
depart_button = Pin(DEPART_BUTTON_PIN, Pin.IN, Pin.PULL_UP)
...
target_station = BUS_NUM        # bus N starts heading to station N
bus_status = "Moving"
seconds_left = TRAVEL
...
print("Stop time:", DWELL, "s")
print("First stop: Station", target_station)
print("Buttons: pin 4 = ARRIVE , pin 5 = DEPART")
...
while True:
    ...
    current_arrive_state = arrive_button.value()
    current_depart_state = depart_button.value()

    # =====================================================
    # ARRIVE BUTTON
    # =====================================================

    if last_arrive_state == 1 and current_arrive_state == 0:

        if bus_status == "Moving":
            print()
            print(">> ARRIVE BUTTON - skipping travel time")
            seconds_left = 0
        else:
            print(">> ARRIVE BUTTON ignored - bus is already at the stop")

    # =====================================================
    # DEPART BUTTON
    # =====================================================

    if last_depart_state == 1 and current_depart_state == 0:

        if bus_status == "Arrived":
            print()
            print(">> DEPART BUTTON - leaving the stop now")
            seconds_left = 0
        else:
            print(">> DEPART BUTTON ignored - bus is not at a stop")
    ...
    last_arrive_state = current_arrive_state
    last_depart_state = current_depart_state
